...
describe('SignIn', () => {
  ...
  it('should throw an error if username length is less than minimum length', () => {
    const req: Request = authMockRequest({}, { username: WRONG_PASSWORD, password: WRONG_PASSWORD }) as Request;
    const res: Response = authMockResponse();
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(error.statusCode).toEqual(400);
  ...
  it('should throw an error if password length is less then minimum length', () => {
  ...
  it('should throw an error if password length is greater then maximum length', () => {
  ...
  it('should throw "Invalid credentials" if username does not exist', () => {
    const req: Request = authMockRequest({}, { username: WRONG_USERNAME, password: LONG_PASSWORD }) as Request;
    const res: Response = authMockResponse();
    jest.spyOn(authServices, 'getAuthUserByUsername').mockResolvedValueOnce(null as any);

    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      expect(authServices.getAuthUserByUsername).toHaveBeenCalledWith(Helpers.firstLatterUppercase(req.body.username));
      expect(error.statusCode).toEqual(400);
  ...
  it('should set session data for valid credentials and send correct json response', async () => {
    ...
    jest.spyOn(authServices, 'getAuthUserByUsername').mockResolvedValueOnce(authMock);
