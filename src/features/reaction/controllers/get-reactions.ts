...
const reactionChache: ReactionsCache = new ReactionsCache();

export class Get {
  public async reaction(req: Request, res: Response): Promise<void> {
    const { postId } = req.params;
    const cachedReactions: [IReactionDocument[], number] =
      await reactionChache.getReactionFromCache(postId);
  ...
  public async singleReactionByUsername(
    ...
    const cachedReaction: [IReactionDocument, number] | [] =
      await reactionChache.getSingleReactionByUsernameFromCache(
        postId,
        username
      );
