...
export class Get {
  public async comments(req: Request, res: Response): Promise<void> {
    ...
    const comments: ICommentDocument[] = cachedComments.length ? cachedComments : await commentService.getPostComments({ postId: new mongoose.Types.ObjectId(postId) }, { createAt: - 1 });
  ...
  public async commentsNamesFromCache(req: Request, res: Response): Promise<void> {
    ...
    const commentsNames: ICommentNameList[] = cachedCommentsNames.length ? cachedCommentsNames : await commentService.getPostCommentNames({ postId: new mongoose.Types.ObjectId(postId) }, { createAt: - 1 });
  ...
  public async singleComment(req: Request, res: Response): Promise<void> {
    ...
    const comments: ICommentDocument[] = cachedComments.length ? cachedComments : await commentService.getPostComments({ _id: new mongoose.Types.ObjectId(commentId) }, { createAt: - 1 });
